...
def darksearchsearch(searchstr):
    results = []
    darksearchserver_url1 = supported_engines['darksearchserver'] + "/oss/"
    darksearchserver_url2 = None
    results_per_page = 10
    max_nb_page = 100
    if args.limit != 0:
        max_nb_page = args.limit
    ...
    with requests.Session() as s:
        ...
        resp = s.get(darksearchserver_url1)
        soup = BeautifulSoup(resp.text, 'html5lib')
        for i in soup.find_all('iframe', attrs={"style": "display:none;"}):
            darksearchserver_url2 = i['src'] + "{}&page={}"

        if darksearchserver_url2 is None:
            return results

        resp = s.get(darksearchserver_url2.format(quote(searchstr), 1))
        soup = BeautifulSoup(resp.text, 'html5lib')
        ...
        pos = get_proc_pos()
        with tqdm(total=page_number, initial=0, desc=get_tqdm_desc("dark Search Server", pos), position=pos) \
                as progress_bar:

            results = link_finder("darksearchserver", soup)
            progress_bar.update()

            for n in range(2, page_number + 1):
                resp = s.get(darksearchserver_url2.format(quote(searchstr), n))
                soup = BeautifulSoup(resp.text, 'html5lib')
                results = results + link_finder("darksearchserver", soup)
                progress_bar.update()
...
def darksearchengine(searchstr):
    results = []
    darksearchengine_url = supported_engines['darksearchengine'] + "/search.php?search={}&submit=Search&page={}"
    # same as darksearchengine_url = "http://5u56fjmxu63xcmbk.onion/search.php?search={}&submit=Search&page={}"
    max_nb_page = 100
    if args.limit != 0:
        max_nb_page = args.limit
    ...
    with requests.Session() as s:
        ...
        resp = s.get(darksearchengine_url.format(quote(searchstr), 1))
        soup = BeautifulSoup(resp.text, 'html5lib')
        ...
        pos = get_proc_pos()
        with tqdm(total=page_number, initial=0, desc=get_tqdm_desc("dark Search Engine", pos), position=pos) \
                as progress_bar:

            results = link_finder("darksearchengine", soup)
            progress_bar.update()

            for n in range(2, page_number + 1):
                resp = s.get(darksearchengine_url.format(quote(searchstr), n))
                soup = BeautifulSoup(resp.text, 'html5lib')
                results = results + link_finder("darksearchengine", soup)
                progress_bar.update()
